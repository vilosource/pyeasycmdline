...
class CommandContextFilter(logging.Filter):
    ...
    def filter(self, record):
        record.command_path = ".".join(self.command_path) if self.command_path else "main"
        return True
...
def setup_logging(config: Dict[str, Any]) -> None:
    ...
    default_level_name = (
        log_config.get("default_level") or 
        os.environ.get("EASYCMDLINE_LOG_LEVEL") or 
        DEFAULT_LOG_LEVEL
    )
...
def get_command_logger(command_path: list, config: Dict[str, Any]) -> logging.Logger:
    ...
    logger_name = f"easycmdline.command.{'.'.join(command_path)}" if command_path else "easycmdline"
