...
class EasyCmdLine:
    ...
    def __init__(self, config_file=None):
        ...
        """
        self.config_file = config_file or os.environ.get('EASYCMDLINE_CONFIG', 'easycmdline-config.yml')
    ...
    def create_subparsers(self, parser, commands, parents=None):
        ...
        self.logger.debug(f"Creating subparsers for commands: {list(commands.keys())} with parents: {parents}")
        subparsers = parser.add_subparsers(dest='command' if not parents else f"command_{len(parents)}")
    ...
    def execute(self, args=None):
        ...
        commands = {k: v for k, v in self.config.get("commands", {}).items() if k != "command"}
        ...
        cmd_logger = get_command_logger(args._command_path if hasattr(args, '_command_path') else [], self.config)
        ...
        try:
            ...
            cmd_logger.info(f"Executing command: {' '.join(args._command_path) if hasattr(args, '_command_path') else args.command}")
